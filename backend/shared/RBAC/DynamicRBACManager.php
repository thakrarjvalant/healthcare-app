...
class DynamicRBACManager
{
    ...
    public function hasPermission($userId, $permission, $resource = null)
    {
        ...
        $sql = "SELECT COUNT(*) FROM user_dynamic_roles udr
                ...
                WHERE udr.user_id = ? AND udr.is_active = TRUE 
                AND drp.is_active = TRUE AND dp.name = ?";
    ...
    public function getUserRoles($userId)
    {
        $sql = "SELECT dr.*, 
                ...
                LEFT JOIN dynamic_role_permissions drp ON dr.id = drp.role_id AND drp.is_active = TRUE
                LEFT JOIN dynamic_permissions dp ON drp.permission_id = dp.id
                WHERE udr.user_id = ? AND udr.is_active = TRUE AND dr.is_active = TRUE
                GROUP BY dr.id";
    ...
    public function getRoleFeatureAccess($roleId)
    {
        $sql = "SELECT rfa.*, fm.name as module_name, fm.display_name as module_display_name
                ...
                WHERE rfa.role_id = ? AND rfa.is_active = TRUE AND fm.is_enabled = TRUE
                ORDER BY fm.name";
    ...
    {
        $sql = "SELECT * FROM dynamic_permissions WHERE is_active = TRUE ORDER BY name";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(\PDO::FETCH_ASSOC);
    }
    ...
    public function getRolePermissions($roleId)
    {
        $sql = "SELECT dp.* FROM dynamic_role_permissions drp
                JOIN dynamic_permissions dp ON drp.permission_id = dp.id
                WHERE drp.role_id = ? AND drp.is_active = TRUE AND dp.is_active = TRUE
                ORDER BY dp.name";
    ...
    public function assignFeatureAccessToRole($roleId, $moduleId, $accessLevel, $assignedBy)
    {
        ...
        $sql = "SELECT id FROM role_feature_access 
                WHERE role_id = ? AND module_id = ? AND is_active = TRUE";
        $stmt = $this->db->prepare($sql);
    ...
    private function clearRoleCache($roleId)
    {
        ...
        $sql = "SELECT user_id FROM user_dynamic_roles WHERE role_id = ? AND is_active = TRUE";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$roleId]);
